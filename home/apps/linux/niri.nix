...
in
{
  ...
  config = lib.mkIf cfg.enable {
    assertions = [
      {
        assertion = config.app."noctalia-shell".enable;
        message = "app.niri.enable requires app.\"noctalia-shell\".enable.";
      }
    ];
    ...
    programs.niri.settings = {
      ...
      binds = {
        "Mod+Return" = lib.mkIf config.app.ghostty.enable {
        ...
        "Mod+B" = lib.mkIf config.app.firefox.enable {
          action.spawn = [ "firefox" ];
        };
        "Mod+E" = lib.mkIf config.app.vscode.enable {
          action.spawn = [ "code" ];
        };
        "Mod+N" = lib.mkIf config.app.nautilus.enable {
          action.spawn = [
            "nautilus"
            "--new-window"
          ];
